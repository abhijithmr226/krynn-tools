...
export default function Sha256GeneratorTool({
  ...
  const generate = () => {
    if (!input) return;
    setHash(CryptoJS.SHA256(input).toString());
    setCopied(false);
  };
  ...
  return (
    ...
    >
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-6">
        ...
        {hash && (
          <div className="mt-4">
            <label className="mb-2 block text-sm font-medium text-[var(--color-foreground)]">SHA-256 Hash</label>
            <div className="flex items-center gap-2">
              <div className="flex-1 rounded-lg border border-[var(--color-border)] bg-[var(--color-background)] px-4 py-3 font-mono text-xs break-all text-[var(--color-foreground)]">
                {hash}
              </div>
              <button
                onClick={handleCopy}
